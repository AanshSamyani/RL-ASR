...
Inspired by DeepSeek-R1's GRPO but adapted for ASR-TTA where we have a
small group of candidates per utterance and need fast, stable updates.
"""
...
class GRPO:
    ...
    - Clips gradients to prevent large updates
    ...
    def __init__(
        ...
        self._ref_log_probs: list[torch.Tensor] | None = None
    ...
    def store_reference_log_probs(self, candidates: list[dict]) -> None:
        """Snapshot log probs from current policy as KL reference."""
        self._ref_log_probs = [
    ...
    def _normalize_advantages(self, rewards: torch.Tensor) -> torch.Tensor:
        ...
        Produces zero-mean, unit-variance advantages for stable gradients.
        """
        mean = rewards.mean()
        std = rewards.std()
        if std < 1e-8:
    ...
    def compute_loss(
        ...
        for i in range(n):
            if self.token_level and len(candidates[i]["log_probs"]) > 0:
                pg_loss = pg_loss + (-advantages[i] * candidates[i]["log_probs"].sum())
            else:
                pg_loss = pg_loss + (-advantages[i] * candidates[i]["total_log_prob"])

            if self._ref_log_probs is not None and i < len(self._ref_log_probs):
                kl_loss = kl_loss + self._compute_token_kl(
